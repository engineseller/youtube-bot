...
def do_process(driver, urls, comment):
    if len(urls) == 0:
    ...
    if not check_exists_by_xpath(driver, '//*[@id="movie_player"]'):
        return do_process(driver, urls, random_comment())
    ...
    # driver.execute_script('document.querySelector("#movie_player").scrollIntoViewIfNeeded();')
    driver.execute_script("window.scrollTo(0, window.scrollY + 500)")
    time.sleep(2)

    # finding comment box and submit comment
...
def random_comment():
    ...
    try:
        with open(file, 'r') as rfile:
            ...
            for i in data:
                if i not in comments:  # check duplicate
                    comments.append(i)
            rfile.close()
    except IOError:
    ...
    try:
        with open(file, 'r') as rfile:
            ...
            for account in accounts:
                ...
                do_process(driver, urls, random_comment())
                driver.quit()
                time.sleep(2)
            rfile.close()
    except IOError:
